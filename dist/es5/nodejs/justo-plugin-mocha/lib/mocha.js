...
function mocha(params) {
  ...
  //(2) determine command
  if (/^win/.test(_os2["default"].platform())) cmd = "mocha.cmd";else cmd = "mocha";
  ...
  if (params.requires) {
    ...
    try {
      for (var _iterator = params.requires[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        var r = _step.value;
  ...
  try {
    for (var _iterator2 = params.files[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
      var f = _step2.value;
      args.push(f);
    } //(3) run
  } catch (err) {
